...
The apps, including AI models (other than in *Ask Azure*), run in your local browser and no data is shared with Microsoft. No data from your browser, such as cookies or configuration data, is collected by any of these apps.
...
- In **Speech Playground**, when <u>not</u> using a generative AI model, keywords from prompts are sent to the [Wikipedia API](https://en.wikipedia.org/w/api.php) to retrieve relevant information from Wikipedia. Only text you explicitly enter into the chat API is sent to Wikipedia.
...
- In **Ask Azure**, input is sent to your Foundry model endpoint. Additionally, when using speech, your spoken input is sent to Azure Speech-to-Text in your Foundry project (and the response is generated by Azure Text-to-speech)

### Generative AI

The **Ask Azure** app uses a model that you choose to deploy in your Microsoft Foundry resource. We recommend deploying a [GPT 4.1 Mini](https://ai.azure.com/catalog/models/gpt-4.1-mini) model. When used in Microsoft Foundry, default content safety guardrails are applied to mitigate the risk of offensive or harmful content generation.
